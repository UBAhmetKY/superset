...
describe('sqlLabReducer', () => {
  ...
  describe('Query editors actions', () => {
    ...
    test('should remove a query editor including unsaved changes', () => {
      ...
      newState = sqlLabReducer(newState, setDbAction);
      expect(newState.unsavedQueryEditor.dbId).toEqual(setDbAction.dbId);
      const removeAction = {
        type: actions.REMOVE_QUERY_EDITOR,
        queryEditor: qe,
      } as SqlLabAction;
  ...
  describe('Tables', () => {
    ...
    test('should merge the table attributes', () => {
      ...
      expect(newState.tables).toHaveLength(1);
      expect(
        (newState.tables[0] as unknown as Record<string, unknown>).extra,
      ).toBe(true);
    });
